...
function MemberContent() {
  ...
  const { data: members = [], isLoading } = trpc.members.list.useQuery(undefined, { enabled: isAdmin });
  ...
  return <div className="admin-page members-page">
    ...
    <section className="member-table-wrap">{isLoading ? <div className="dashboard-loading">회원 정보를 불러오는 중입니다.</div> : members.length ? <table className="member-table"><thead><tr><th>회원</th><th>로그인 수단</th><th>역할</th><th>가입일</th><th>최근 로그인</th></tr></thead><tbody>{members.map(member => <tr key={member.id}><td><strong>{member.name || "이름 없음"}</strong><span>{member.email || "이메일 정보 없음"}</span></td><td>{member.loginMethod || "Manus 계정"}</td><td><mark className={member.role === "admin" ? "role-pill role-pill--admin" : "role-pill"}>{member.role === "admin" ? "운영자" : "회원"}</mark></td><td>{new Date(member.createdAt).toLocaleDateString("ko-KR")}</td><td>{new Date(member.lastSignedIn).toLocaleDateString("ko-KR")}</td></tr>)}</tbody></table> : <div className="analytics-empty"><UsersRound size={25} /><p>아직 가입·로그인을 선택한 회원이 없습니다.</p></div>}</section>
  </div>;
}
