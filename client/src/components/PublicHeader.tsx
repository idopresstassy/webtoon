...
export default function PublicHeader({ backTo }: { backTo?: string }) {
  ...
  return (
    <header className="public-header">
      ...
      {menuOpen && <nav className="mobile-nav-panel" aria-label="모바일 메뉴"><Link href="/" onClick={() => setMenuOpen(false)}>작품 둘러보기</Link><Link href="/admin" onClick={() => setMenuOpen(false)}>운영자 로그인</Link></nav>}
    </header>
  );
}
